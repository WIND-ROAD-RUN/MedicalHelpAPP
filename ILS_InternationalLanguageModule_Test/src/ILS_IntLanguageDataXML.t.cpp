	...
	namespace ILS {
		...
		namespace INT_LANGUAGE_DATA_XML {
			...
			TEST(FIRST_DEEP, CHANGE_STRING)
			{
				...
				EXPECT_EQ(result, IntLanguageDataAbstract::ErrorInfo::SUCCESS) << "将文件中查找字符串失败";
...
//				result = test->storeString("1", "CHN", s2);
//
//				EXPECT_NE(result, IntLanguageDataAbstract::ErrorInfo::SUCCESS) << "将字符串s2存储到文件中异常实现";
//
//				result = test->searchString("1", "CHN", s4);
//
//				EXPECT_EQ(s3, s4) << "对同一个id和language读入不同字符串异常实现";
//
//				result = test->delString("1", "CHN");
//
//				EXPECT_EQ(result, IntLanguageDataAbstract::ErrorInfo::SUCCESS) << "在文件中删除字符串失败";
